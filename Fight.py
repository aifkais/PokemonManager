...
def onematch(turniername, trainer1, gen1, trainer2, gen2, allespieler):
  ...
  if trainer1 in allespieler and trainer2 in allespieler:
    ...
    copy_file(trainer1 + ".txt",trainer2 + ".txt")
...
def makestats(pokenames, gen, statslist):
  ...
  for i in range(len(pokenames)):
    ...
    tempstat = linetoArr(
        "PokeStats", "Species", pokenames[i],
        tempstat)  #profile = linetoArr("Spieler", "NutzerID",str(id),profile)
...
def getday():  #getcell("Trainer"+str(gen)+"Gen","Lose","TrainerName",Spielerliste[i])
  z1 = getcell("Spieltage", "Tag", "ROWID", str(1))
  z2 = getcell("Spieltage", "Transfertag", "ROWID", str(1))
  return z1 , z2
...
def resetseason():
  ...
  for i in range(5):
    coltoreset("SpieltagGen1l1", "Field" + str(i + 1), "NULL")
    coltoreset("SpieltagGen1l2", "Field" + str(i + 1), "NULL")
    coltoreset("SpieltagGen1l3", "Field" + str(i + 1), "NULL")
