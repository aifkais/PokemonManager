...
def spieltag(tag):
  ...
  pastresults11 = getalldb("SpieltagGen1l1", pastresults11)
  pastresults12 = getalldb("SpieltagGen1l2", pastresults12)
  pastresults13 = getalldb("SpieltagGen1l3", pastresults13)
  allespieler=[]
  ...
  if spieltage2[0] == 1:
    ...
    for i in range(5):
      wintodb("SpieltagGen1l1", "Field" + str(i + 1), result11[i], "ROWID",
              tag)
      wintodb("SpieltagGen1l2", "Field" + str(i + 1), result12[i], "ROWID",
              tag)

      wintodb("SpieltagGen1l3", "Field" + str(i + 1), result13[i], "ROWID",
              tag)
  ...
  if spieltage2[5]:
    ...
    print("Transfertag")
    spieler = []
    spieler = getalldb("Spieler", spieler)
    #print()
    #print(table1)
    #print()
    #print(spieler)
...
def pokeaccess1(trainername, gen, pokemon1, pokemon2):
  ...
  spielerpkm = []
  spielerpkm = getalldb("Spieler", spielerpkm)
...
#players = ["Spieler " + str(i+1) for i in range(9)]
#print(tGen1L1)
#print(tGen1L2)
#print(tGen1L3)

#print(tGen1L1)
#schedule = generate_round_robin(tGen1L1,3)
#print_schedule(schedule)

#spieltag(1)
#spieltag(19)

#onematch("Liga 1","juggler",2,"juggler",2)

#pokeaccess1("lass",3,"SPECIES_CHIMECHO","SPECIES_CACTURNE")
"""
def generate_round_robin(players,gen,spieltag):
    if len(players) % 2 != 0:
        players.append("Bye")
    num_rounds = len(players) - 1
    erg = []
    ...
    for round_num in range(spieltag-1):
        players = [players[0]] + [players[-1]] + players[1:-1]
    for i in range(len(players) // 2):
        erg.append(onematch("Liga 2",players[i],gen,players[-i-1],gen))
        round_matchups.append((players[i], players[-i-1]))
    #matchups.append(round_matchups)
    return erg
"""
